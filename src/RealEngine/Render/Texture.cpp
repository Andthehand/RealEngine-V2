...
namespace RealEngine {
	...
	Texture2DArray::Texture2DArray(const Texture2DArrayCreateInfo& info, const void** data)
		: m_Width(info.Width), m_Height(info.Height), m_Length(info.NumTextures), m_InternalFormat(info.InternalFormat), m_DataFormat(info.DataFormat) {
		RE_PROFILE_FUNCTION();

		RE_CORE_ASSERT(info.NumTextures > 0, "Cannot create TextureArray with no textures");
		RE_CORE_ASSERT(m_Width > 0 && m_Height > 0 && m_Length > 0, "Width, Height, and length must be greater than 0 for TextureArray");
	...
	template<class It>
	void Texture2DArray::LoadTextures(It begin, uint32_t mipLevels) {
		RE_PROFILE_FUNCTION();

		RE_CORE_ASSERT(m_Length != 0, "Can't make Texuture without any textures");
		TextureData* textureData = new TextureData[m_Length];
		stbi_uc** data = new stbi_uc * [m_Length];
		...
		CreateTextureArray((const void**)data, mipLevels);
	...
	void Texture2DArray::CreateTextureArray(const void** data, uint32_t mipLevels) {
		RE_PROFILE_FUNCTION();

		//Create TextureArray and allocate memory
		glCreateTextures(GL_TEXTURE_2D_ARRAY, 1, &m_RendererID);
		glTextureStorage3D(m_RendererID, mipLevels, (GLenum)m_InternalFormat, m_Width, m_Height, m_Length);
		
		//Upload the images to the GPU
		if (data != nullptr) {
			for (uint32_t i = 0; i < m_Length; i++) {
				glTextureSubImage3D(m_RendererID, 0, 0, 0, i, m_Width, m_Height, 1, (GLenum)m_DataFormat, GL_UNSIGNED_BYTE, data[i]);
			}
		}
	...
	void Texture2DArray::SetSubTextureData(const void* data, uint32_t width, uint32_t height, uint32_t zOffset) {
		RE_PROFILE_FUNCTION();
		...
		RE_CORE_ASSERT(width <= m_Width && height <= m_Height, "Width and height must not exceed texture array dimensions")
		RE_CORE_ASSERT(data != nullptr, "Data pointer cannot be null when setting sub texture data");
		...
		glTextureSubImage3D(m_RendererID, 0, 0, 0, zOffset, width, height, 1, (GLenum)m_DataFormat, Utils::GetGLType(m_InternalFormat), data);
	}
